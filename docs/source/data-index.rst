...
Each item object should have an id and a field for each filter field and encoder field in your ``init_schema`` configuration.
...
.. note::
   Check out the ``init_schema`` :doc:`installation<example configuration>`.
...
.. note:: 
   In the example on the :doc:`installation<configuration>` page, indexing an item sold only in the US would affect one partition, whereas indexing an item sold in both the US and EU would affect two partitions.
